...
Озвучка: [есть/нет, мужская/женская, тон, дословно текст звучки, разбить на спикеров]
...
async def analyze_video_gemini(video_bytes: bytes, mime_type: str) -> str:
    ...
    try:
        ...
        # Gemini File API обрабатывает видео не сразу.
        # Используем файл только после статуса ACTIVE.
        for _ in range(60):
            ...
            return "Видео не удалось проанализировать: истекло время ожидания обработки Gemini."
        ...
        if uploaded is not None:
            try:
                ...
                logger.warning("Не удалось удалить файл Gemini: %s", delete_err)

        if tmp_path and os.path.exists(tmp_path):
            try:
                os.unlink(tmp_path)
            except Exception as unlink_err:
                logger.warning("Не удалось удалить временный файл: %s", unlink_err)
...
async def cb_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_text = (
        ...
        "Видео: максимум 20 МБ для обычного Telegram Bot API. "
        "Если файл больше — его нужно сжать до 20 МБ или подключить локальный Telegram Bot API Server."
    )
        ...
        for idx, (file_id, mime_type, file_size) in enumerate(video_snap, 1):
            try:
                ...
                if file_size and file_size > max_video_size:
                    ...
                    analyses.append(
                        f"Видео {idx}: не удалось обработать — файл {size_mb} МБ, "
                        "лимит обычного Telegram Bot API — 20 МБ. "
                        "Сожмите видео до 20 МБ или подключите локальный Telegram Bot API Server."
                    )
                    continue
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if msg.video:
        video_buf.append((msg.video.file_id, msg.video.mime_type or "video/mp4", msg.video.file_size))
    elif msg.document and msg.document.mime_type and msg.document.mime_type.startswith("video/"):
        video_buf.append((msg.document.file_id, msg.document.mime_type, msg.document.file_size))
    elif is_media:
