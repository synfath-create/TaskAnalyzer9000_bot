...
# ── Промпты ───────────────────────────────────────────────────────────────────
GEMINI_VIDEO_PROMPT = """Ты анализируешь рекламный видео-креатив с позиции AI-креатора и performance creative production. Без вводных фраз, без смайлов. Не додумывай факты: если параметр невозможно определить по видео — пиши «не определено».
...
Визуал: [локация, освещение, цветовая гамма, стиль, композиция, как встроен стример/персонаж/слот]
...
- Основные фразы дословно: [если слышно]
...
- Источник геймплея: [реальная запись / сгенерированная имитация / UI-мокап / не определено]
...
def _analyze_video_gemini_sync(video_bytes: bytes, mime_type: str) -> str:
    ...
    try:
        ...
        response = gemini_client.models.generate_content(
            model=GEMINI_MODEL,
            contents=[uploaded, GEMINI_VIDEO_PROMPT],
        )
